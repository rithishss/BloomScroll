...
1. **Land on `/`.** Point out the headline and the two CTAs — "Upload your first PDF" (real mode) and "Try the interactive demo" (no signup, no keys). Scroll to the "Grounded by design" section to show a real source excerpt turning into a reel.
...
3. **Watch and swipe through 2–3 reels.** Each one is a real, playable, narrated vertical video — title/topic/explanation baked into the frame, read aloud, with a Ken Burns pan. Show:
   - Dragging the reel left/right (or clicking "Review again" / "Got it").
   - The mute toggle and the transcript ("captions") button.
   - The "Why this reel?" info button — a real, inspectable ranking reason, not a canned string.
   - Saving a reel with the bookmark button or the `S` key.
4. **Click "View source" on any reel.** This is the credibility moment: the drawer shows the *exact stored excerpt*, the full passage as stored, the document title, page number, and a working "Open PDF at page N" link into a real (repository-owned, generated) PDF — the reel's narration and on-screen text are grounded in that exact text.
5. **Go to `/demo/saved`.** The reel you just saved is there, with filters by document/type/difficulty and a "Watch" button that reopens it.
...
- Switch on real mode with a Supabase project + OpenAI key (see the README) and upload an actual PDF to show the live processing timeline (extracting → chunking → writing scripts → rendering reels → ready) and a freshly generated, freshly narrated reel set.

## Five likely interview questions

**1. "Why videos instead of just text cards?"**
The product started as a swipeable text-card feed, then the direction shifted to short narrated reels — the more "TikTok-for-your-notes" framing. Architecturally this meant adding a rendering stage after card generation, not replacing it: the same generated title/explanation/takeaway/citations still exist and still drive ranking, mastery, and the source drawer — they're just also rendered into a video frame and read aloud, via `lib/video/`. Keeping the original `StudyCard` type and adding video fields to it (rather than introducing a parallel type) meant the ranking, ownership, and citation infrastructure didn't need to change at all.

**2. "Why not just use cosine similarity + a fixed threshold for ranking diversity instead of a hard filter?"**
A fixed score penalty is a soft preference — it can always be outweighed by a large enough gap in another scoring component. I found this empirically while testing: with an extreme topic-preference gap (explicit weight 1.0 vs 0.0), a fixed penalty of 0.15 wasn't enough to prevent three consecutive same-topic reels, because the raw score gap between the two topics exceeded the penalty. The spec's wording — "avoid repetition when alternatives exist" — is actually a hard constraint, so I implemented it as one: a candidate that would create a third consecutive same-topic/document reel is only eligible when nothing else in the remaining pool avoids it. That's `lib/feed/ranking.ts`, and the reasoning is in the file comment plus the test that caught the original bug.
