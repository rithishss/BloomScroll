 ...
 * dependencies. Exercises the same orchestration code the real job runner
 * uses (lib/documents/job-runner.ts wires the real Supabase/LangChain
...
describe("processDocument — happy path", () => {
  it("processes a document end to end and marks it ready", async () => {
    const db = new FakeDb();
    const result = await processDocument("doc-1", fakeDeps(db));
  ...
  it("advances progress through meaningful milestones", async () => {
    const db = new FakeDb();
    ...
    await processDocument("doc-1", fakeDeps(db));
...
describe("processDocument — retry / idempotency", () => {
  ...
  it("is safe to run twice in a row and reach the same ready state", async () => {
    ...
    const second = await processDocument("doc-1", fakeDeps(db));
...
describe("processDocument — failure handling", () => {
  it("records a user-friendly message and 'failed' status on extraction failure", async () => {
    ...
    const result = await processDocument("doc-1", deps);
